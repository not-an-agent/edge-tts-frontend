...
class SocketEdgeTTS {
			...
			if (data.includes("Path:turn.end")) {
				...
				for (let _ind = 0; _ind < this.audios.length; _ind++) {
					...
					if (parts.length > 0 && parts[0] instanceof Blob) {
						...
						const combinedUint8Array = new Uint8Array(this.my_uint8Array.length + uint8_Array2.length)
						combinedUint8Array.set(this.my_uint8Array, 0)
						combinedUint8Array.set(uint8_Array2, this.my_uint8Array.length)
						this.my_uint8Array = combinedUint8Array
					}
				}
				this.save_mp3()
			}
	...
	async saveFiles(blob) {
		if (!this.start_save) {
			...
			// Capture names synchronously before any async op that might clear state
			const filename = this.my_filename
			const filenum = this.my_filenum
			try {
				const folderHandle = await save_path_handle.getDirectoryHandle(filename, { create: true });
				const fileHandle = await folderHandle.getFileHandle(filename + ' - Part ' + filenum + '.mp3', { create: true });
				...
				link.href = url;
				link.download = filename + ' - Part ' + filenum + '.mp3';
		...
		if (this.my_uint8Array.length > 0) {
			...
			if (!this.save_to_var) {				
				...
				if (save_path_handle ?? false) {
					...
					const link = document.createElement('a');
					link.href = url;
					link.download = this.my_filename + ' - Part ' + this.my_filenum + '.mp3';
					document.body.appendChild(link);
					link.click();
					document.body.removeChild(link);
					window.URL.revokeObjectURL(url);
